...
bool currentbone = true;
bool keyBoneSwitch = 0;
    ...
    if (enable_glow_hack == 1) {
        if ((int)target->buffer[GLOW_CONTEXT] != 1 || (int)target->buffer[GLOW_VISIBLE_TYPE] != 1 || (int)target->buffer[GLOW_FADE] != 872415232) {
            ...
            if (!isnan(currentEntityTime) && currentEntityTime > 0.f) {
                ...
                if (target->getTeamId() == LPlayer->getTeamId()) {
                    color = { 1.65f, 2.28f, 0.87f }; //Teammate - Light green
                }
                else if (target->isBleedOut() || !target->isOkLifeState()) {
                    color = { 3.f, 3.f, 0.f }; //Downed enemy - Yellow
                }
                else {
                    if (target->vis_time() > vis_old[id] || target->vis_time() < 0.f && vis_old[id] > 0.f) {
                        color = { 0.f, 3.f, 0.f }; //Has vision - Green
                    }
                    else {
...
void SmoothType_Asist(float fov, float TargetDistance, Vector* Delta, float smooth_multiplier) {
    ...
    if (TargetDistance < 10.f) {
        smooth = 4.f + (smooth_multiplier - 1.f) * 3.f;
    }
    else {
        smooth = 4.f + (2.f + (smooth_multiplier - 1.f)) * fov;
    }
    if (smooth > 0.1f) {
        Delta->x /= smooth;
        Delta->y /= smooth;
...
void CheatLoop() {
    ...
    while (true) {
        ...
        //Bone Switch
            /*So when you start the cheat its aiming at head until press keybind
             then when you press keybind it switch to body until you press keybind again
             then when you press again switch to head until you press keybind again*/

        bool f6 = (GetAsyncKeyState(VK_PRIOR) & 0x8000) != 0; //set to page up (VK_PRIOR) 
        if (f6 && keyBoneSwitch == 0) { //0 is false
            keyBoneSwitch = 1; //docs.microsoft.com/en-us/windows/win32/inputdev/virtual-key-codes
            currentbone = !currentbone;
            Beep(900, 300);
        }
        else if (keyBoneSwitch == 1) { // 1 is true
            keyBoneSwitch = 0;
        }
